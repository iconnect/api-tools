...
cborTool :: APITool
cborTool = readTool cborTool'

-- | Tool to generate 'Serialise' instances for types generated by
-- 'datatypesTool'. This depends on 'enumTool'.  It may be supplied
-- with an API that is bigger than the fragment being generated.
cborTool' :: API -> APITool
cborTool' api = apiNodeTool $ apiSpecTool gen_sn_to (gen_sr_to napi) (gen_su_to napi) gen_se_to mempty
                    <> gen_pr
  where
...
gen_sn_to = mkTool $ \ ts (an, sn) -> optionalInstanceD ts ''Serialise [nodeRepT an]
  ...
  where
    ...
    ine sn = case snType sn of
            ...
            BTbool   -> [e| encodeBool |]
            BTint    -> [e| encodeInt |]
            BTutc    -> [e| encodeUTCTime' |] -- Not encodeUTCTime, for complicated reasons
...
gen_sr_to :: NormAPI -> Tool (APINode, SpecRecord)
gen_sr_to api = mkTool $ \ ts (an, sr) -> do
    x <- newName "x"
            ...
            encFields =
                varE 'encodeRecordFields `appE`
                    listE [ [e| encodeString $(fieldNameE fn)
                                <> $(encoder api (ftType fty)) ($(nodeFieldE an fn) $(varE x)) |]
                            | (fn, fty) <- fields ]
        in lamE [varP x] $
               varE '(<>)
...
-- | Encode a UTCTime to match Aeson's encoding. Prior to aeson-0.8.1
-- this always included fractions of a second to 3 decimal places.
-- Thereafter it is more selective in whether to include fractions of
-- a second, so we use 'encodeUTCTime'' instead.  This agrees with the
-- api-tools encoding only for times without fractions of a second,
-- but that's good enough.
encodeUTCTime :: UTCTime -> Encoding
#if MIN_VERSION_aeson(0,8,1)
encodeUTCTime = encodeUTCTime'
#else
encodeUTCTime t = encode (parseUTC_ $ formatTime defaultTimeLocale fmt t)
 where
   fmt = "%FT%T." ++ take 3 (formatTime defaultTimeLocale "%q" t) ++ "Z"
#endif

-- | Encode a UTCTime to match 'ToJSON' instances generated by
-- api-tools, which never include fractions of a second.
encodeUTCTime' :: UTCTime -> Encoding
encodeUTCTime' = encode

-- | Generate an encode function of type @t -> Encoding@ given the API
-- and representation of the type.  This is necessary to match Aeson's
-- 'UTCTime' encoding, rather than using the 'Serialise' instance for
-- 'UTCTime'.  We issue a warning and fall back on 'encode' where a
...
gen_su_to :: NormAPI -> Tool (APINode, SpecUnion)
gen_su_to api = mkTool $ \ ts (an, su) -> optionalInstanceD ts ''Serialise [nodeRepT an]
  ...
  where
    ...
    cl an (fn,(ty,_)) = do x <- newName "x"
                           clause [nodeAltConP an fn [varP x]] (bdy fn ty x) []

    bdy fn ty x = normalB [e| encodeUnion $(fieldNameE fn) ($(encoder api ty) $(varE x)) |]
