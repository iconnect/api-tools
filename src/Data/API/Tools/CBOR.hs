...
cborTool' :: APITool
cborTool' = readTool $ \ api ->
              apiNodeTool $ apiSpecTool gen_sn_to (gen_sr_to api) (gen_su_to api) gen_se_to mempty
                              <> gen_pr
...
gen_sr_to :: API -> Tool (APINode, SpecRecord)
...
-- | Encode a UTCTime to match Aeson's encoding, which always includes
-- fractions of a second to 3 decimal places.
...
-- api-tools, which are not the same as 'encodeUTCTime', as they never
...
-- 'UTCTime'.  We quietly fall back on 'encode' where a type
-- declaration is missing from the API.
encoder :: API -> APIType -> ExpQ
...
encoder api (TyName tn)     = case [ anSpec an | ThNode an <- api, anName an == tn ] of
    [ SpSynonym ty ] -> encoder api ty
    []               -> reportWarning ("encoder: missing type declaration for " ++ _TypeName tn)
                          >> [e| encode |]
    _                -> [e| encode |]
...
gen_su_to :: API -> Tool (APINode, SpecUnion)
